...
/** \file
 * Levenberg-Marquardt minimization demo driver.
 * Only the double precision versions are tested here.
 * See the Meyer case for an example of verifying the jacobian 
 */
...
#warning LAPACK not available, some test problems cannot be used
  ...
  if (argc != 2) {
    printf("specify problem:\n");
    for (problem=0; problem < 15; problem++) printf("%d. %s\n", problem, probname[problem]);
    exit(0);
  }
  problem = atoi(argv[1]);
