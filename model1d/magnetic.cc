...
#define r4xa Cr4xa
/*
 * CRUFT no longer need fortran r4x since we have Cr4xa
...
*/
extern "C" void
Cr4xa(const int &N, const Real D[], const Real RHO[],
      const Real MU[], const Real &LAMBDA, const Real P[],
      const std::complex<Real> EXPTH[], const Real &AGUIDE,
      const Real &Q, std::complex<Real> &YA, std::complex<Real> &YB,
      std::complex<Real> &YC, std::complex<Real> &YD);
...
magnetic_amplitude(const int layers, const Real d[],
     ...
     const Real P[], const std::complex<Real> expth[],
     const Real Aguide, const int points, const Real Q[],
     std::complex<Real> Ra[], std::complex<Real> Rb[],
     std::complex<Real> Rc[], std::complex<Real> Rd[])
{
  for (int i=0; i < points; i++) {
    r4xa(layers,d,rho,mu,L,P,expth,Aguide,
...
//    variables calculating S1, S3, COSH and SINH
      Real QSQREL, PI2oLAMBDA;
      Real EPA, EMA, COSB, SINB, LOGH;
      std::complex<Real> S1,S3,COSHS1,COSHS3,SINHS1,SINHS3;

//    completely unrolled matrices for B=A*B update
      std::complex<Real> A11,A12,A13,A14,A21,A22,A23,A24;
      std::complex<Real> A31,A32,A33,A34,A41,A42,A43,A44;
      std::complex<Real> B11,B12,B13,B14,B21,B22,B23,B24;
      std::complex<Real> B31,B32,B33,B34,B41,B42,B43,B44;
      std::complex<Real> C1,C2,C3,C4;

//    variables for translating resulting B into a signal
      std::complex<Real> W11,W12,W21,W22,V11,V12,V21,V22;
      std::complex<Real> DETW;
      std::complex<Real> ZI,ZS,X,Y,SCI,SS,CC;

//    constants
      const std::complex<Real> CR(1.0,0.0);
      const std::complex<Real> CI(0.0,1.0);
      const Real PI4=12.566370614359172;
      const Real PI=3.1415926535897932284626;
...
//     B = I
      B11=std::complex<Real>(1.0,0.0);
      B12=std::complex<Real>(0.0,0.0);
      B13=std::complex<Real>(0.0,0.0);
      B14=std::complex<Real>(0.0,0.0);
      B21=std::complex<Real>(0.0,0.0);
      B22=std::complex<Real>(1.0,0.0);
      B23=std::complex<Real>(0.0,0.0);
      B24=std::complex<Real>(0.0,0.0);
      B31=std::complex<Real>(0.0,0.0);
      B32=std::complex<Real>(0.0,0.0);
      B33=std::complex<Real>(1.0,0.0);
      B34=std::complex<Real>(0.0,0.0);
      B41=std::complex<Real>(0.0,0.0);
      B42=std::complex<Real>(0.0,0.0);
      B43=std::complex<Real>(0.0,0.0);
      B44=std::complex<Real>(1.0,0.0);
...
//    2*cosh/H and 2*sinh/H rather than cosh/H and sinh/H
        A11=Real(0.25)*(COSHS1+COSHS3);
        A21=Real(0.25)*(COSHS1-COSHS3);
        A31=Real(0.25)*(SINHS1*S1+SINHS3*S3);
        A41=Real(0.25)*(SINHS1*S1-SINHS3*S3);
        A13=Real(0.25)*(SINHS1/S1+SINHS3/S3);
        A23=Real(0.25)*(SINHS1/S1-SINHS3/S3);
...
//    Note: reusing A instead of creating CST
      ...
      SCI = CI*Real(cos(-AGUIDE/2.*PI/180.)*sin(-AGUIDE/2*PI/180.));
